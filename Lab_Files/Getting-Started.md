...
**AdventureWorks** wants to evaluate **Microsoft Fabric Real-Time Intelligence** by building a proof-of-concept that monitors website activity as events occur. Rather than connecting to a live production website, the engineering team uses a notebook to generate synthetic web events that simulate customer impressions and clicks.

As the **Data Engineer**, your task is to build the complete streaming analytics pipeline. You begin by creating a dedicated **Fabric Workspace** and an **Eventhouse** to store real-time event data. To ensure interoperability with other Fabric experiences, you enable OneLake Availability. Next, you configure an **Eventstream** with a custom endpoint that receives the synthetic events generated by the notebook. Within the Eventstream, you separate click events and impression events into different destinations so they can be stored independently in the Eventhouse.

To enrich the streaming data, you create a **Lakehouse** and upload reference datasets containing product and product category information. You then create shortcuts between the Lakehouse and Eventhouse and build KQL tables, functions, and update policies that transform raw event data into structured analytical datasets. Finally, you create a **Real-Time Dashboard** to monitor streaming metrics, enable automatic refresh so the visuals stay up to date, and configure a Data Activator rule that can trigger actions when specified thresholds are reached.

By the end of the lab, you have implemented an end-to-end **Microsoft Fabric Real-Time Intelligence solution** that ingests, transforms, stores, visualizes, and monitors streaming web events.

## 📖 Overview

In this lab, you will build an end-to-end **Real-Time Intelligence solution in Microsoft Fabric** using simulated website clickstream data. You will create the required Fabric resources, configure an Eventhouse and Eventstream, and use a notebook to generate synthetic streaming events.

You will then create a Lakehouse to store reference data, establish shortcuts between the Lakehouse and Eventhouse, and build KQL tables and transformation logic to organize incoming events. Finally, you will create a **Real-Time Dashboard** that visualizes the streaming data, enables automatic refresh for live monitoring, and configures Data Activator to respond to selected conditions in the incoming event stream.
...
1. Copy the link below and paste it in the address bar, and hit enter to navigate to the sign-in page of the **Microsoft Fabric portal**.
...
1. You will be navigated back to the **Microsoft** **Power BI Home page**. Click on the **Power BI (1)** icon in the bottom left corner, and select **Fabric (2)**. 

    ![](media/gs-step11-1606.png)

    ![](media/gs-step11a-1606.png)    
...
Click **Next >>** from the page navigation menu at the bottom to embark on your Lab journey!
